...
surv_curves = function(hazard, G, col) {
  ...
  up = exp(-exp(log_cum_hazard + 1.96*sqrt(diag(G %*% Sigma_star %*% t(G)))))
  lo = exp(-exp(log_cum_hazard - 1.96*sqrt(diag(G %*% Sigma_star %*% t(G)))))
  segments(grid, lo, grid, up, col="grey95", lwd=2)
  lines(grid, surv, col=col, lwd=2)}


surv_curves_ci = function(theta, beta, col1, col2) {
  ...
  surv_curves(hazard, G, col1)
  
  ############### Hazard with z = 1 ("treated") ###############
  hazard=exp(B_sm %*% theta + beta)
  G[ ,(K+1)] = 1
  surv_curves(hazard, G, col2)
}
########################################################################################
